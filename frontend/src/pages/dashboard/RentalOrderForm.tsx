...
export default function RentalOrderForm() {
  ...
  const rentalPeriod = order.items?.[0]?.start && order.items?.[0]?.end ? `${new Date(order.items[0].start).toLocaleDateString()} - ${new Date(order.items[0].end).toLocaleDateString()}` : '';
  const rentalDuration = order.items?.[0]?.start && order.items?.[0]?.end ? Math.ceil((new Date(order.items[0].end).getTime() - new Date(order.items[0].start).getTime())/(1000*60*60*24)) + ' days' : '';
